...
float vn(vec2 p, float s, float seed) {
  ...
  float bx = mix(g1, g2, u.x);
  float tx = mix(g4, g3, u.x);
  return mix(bx, tx, u.y);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (uUseBg > 0.5) {
    float intensity = clamp(ltn + mGlow * 0.75, 0.0, 1.0);
    vec3 outc = mix(uBgColor, col, intensity);
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    const effectiveDpr = dpr !== undefined ? dpr : (isMobile ? 0.75 : 1);
    ...
    const bgRGB = backgroundColor ? hexToRGB(backgroundColor) : [0, 0, 0];
    const useBg = backgroundColor ? 1.0 : 0.0;
    ...
    let lastH = 0;
    const resize = () => {
      const w = Math.round(container.clientWidth || container.getBoundingClientRect().width);
      const h = Math.round(container.clientHeight || container.getBoundingClientRect().height);
    ...
    // 45 FPS on mobile saves thermals & battery while keeping fluid motion velvety smooth; 60 FPS on desktop
    const targetFps = isMobile ? 45 : 60;
    const fpsInterval = 1000 / targetFps;
    const loop = (t: number) => {
      rafRef.current = requestAnimationFrame(loop);
      if (!lastRenderTime) lastRenderTime = t;
      const elapsed = t - lastRenderTime;
      if (elapsed < fpsInterval) return;
      lastRenderTime = t - (elapsed % fpsInterval);
      ...
      if (!paused && isVisible && isIntersecting && programRef.current && meshRef.current) {
        try {
